...
import { ListPlus, Trash2, PlayCircle, ListMusic, X, PlusSquare, Tag, Sparkles } from 'lucide-react';
...
const ContextMenu: React.FC<ContextMenuProps> = ({ visible, x, y, songId, playlistId, onClose }) => {
  ...
  const { playlists, addToPlaylist, removeFromPlaylist, deleteSong, settings, library, playNext, openCreatePlaylistModal, tagSongsWithAi, setTagEditorSongId } = usePlayer();
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <div
          onClick={() => { setTagEditorSongId(songId); onClose(); }}
          className="flex items-center gap-3 px-4 py-3 hover:bg-white/10 cursor-pointer transition"
        >
          <Tag size={16} /> {t(settings.language, 'editTags')}
        </div>

        <div
          onClick={() => { tagSongsWithAi([songId]); onClose(); }}
          className="flex items-center gap-3 px-4 py-3 hover:bg-white/10 cursor-pointer transition text-cyan-400"
        >
          <Sparkles size={16} /> {t(settings.language, 'aiPerception')}
        </div>
