...
import { Home, Search, Library, PlusSquare, Heart, Settings, ListMusic, Trash2, DownloadCloud, List } from 'lucide-react';
import { usePlayer } from '../context/PlayerContext';
import { NavView, Playlist } from '../types';
import { t } from '../utils/i18n';

const Sidebar: React.FC = () => {
    const { addToQueue, navView, setNavView, playlists, library, createPlaylist, setActivePlaylist, activePlaylistId, deletePlaylist, settings, setNeteaseImportOpen, showConfirm } = usePlayer();
        ...
        <div className="w-64 bg-sidebarBg h-full flex flex-col p-6 border-r border-borderColor flex-shrink-0 hidden md:flex z-20 relative pb-32 glass-panel transition-colors duration-300">
                            ...
                            >
                                ...
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        showConfirm(
                                            t(settings.language, 'confirmDelete'),
                                            `确定要永久删除歌单 "${pl.name}" 吗？此操作不可撤销。`,
            ...
            <div className="mt-auto space-y-4">
                ...
                <div className="border-t border-borderColor pt-4 space-y-2">
                    <button
                        onClick={() => setNeteaseImportOpen(true)}
                        className="w-full text-xs border border-blue-500/30 bg-blue-500/5 rounded-full px-3 py-2 text-blue-400 hover:border-blue-500 hover:bg-blue-500/10 transition flex items-center justify-center gap-2"
                    >
                        <DownloadCloud size={14} /> 导入网易云歌单
                    </button>
                    <button
