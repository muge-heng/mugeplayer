...
import { Clock, Play, Music, Mic2, ArrowUpDown } from 'lucide-react';
...
const SongList: React.FC = () => {
  const { queue, currentSong, play, isPlaying, sortOption, setSortOption, settings } = usePlayer();
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-32 px-6">
       ...
       <div className="flex items-center justify-between mt-4 mb-2 px-4">
            <h2 className="text-xl font-bold text-white">{t(settings.language, 'library')}</h2>
            <div className="flex items-center gap-2 text-sm text-gray-400">
                ...
                >
                    <option value="dateAdded">{t(settings.language, 'sortDate')}</option>
                    <option value="title">{t(settings.language, 'sortTitle')}</option>
                    <option value="artist">{t(settings.language, 'sortArtist')}</option>
                </select>
            </div>
       </div>
